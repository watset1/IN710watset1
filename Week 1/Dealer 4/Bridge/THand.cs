...
namespace Bridge
{
    ...
    class THand
    {
      ...
      public void printHand(ListBox box)
      {
        ...
        // print the card face and value - this needs to be changed*****
        for (int i = 0; i < HANDSIZE; i++)
        {
          box.Items.Add(handArray[i].ToString());
        }
      ...
      public void alternatePrintHand(ListBox box)
        {
            box.Items.Add(" ----------------- ");
            box.Items.Add(mySeat + ":");
            ...
            for (int i = 0; i < HANDSIZE; i++)
            {
                int tmpRank = handArray[i].Rank;

                getRank(tmpRank);
                if (handArray[i].Suit == 3)
                {                 
                    spades += newRank;
                }
                if (handArray[i].Suit == 2)
                {
                    hearts += newRank;
                }
                if (handArray[i].Suit == 1)
                {
                    diamonds += newRank;
                }
                if (handArray[i].Suit == 0)
                {
                    clubs += newRank;
                }
            }
      ...
      public int getHCP()
      {
        ...
        public string getRank(int tmpRank)
        {
